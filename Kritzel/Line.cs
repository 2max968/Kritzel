...
namespace Kritzel
{
    public class LPoint
    {
        ...
        public QubicSpline Spline { get; set; } = null;
    ...
    public class Line
    {
        ...
        void sc(Renderer.BaseRenderer g, LPoint p1, LPoint p2, float quality, bool highlight, float border = 0)
        {
            ...
            if (p1.Spline != null)
            {
                ...
                for (float i = 0; i < 1; i += 1f / n)
                {
                    PointF pf = p1.Spline.GetPoint(i);
                    LPoint p = new LPoint(pf.X, pf.Y, 4);
                    if (border == 0)
                        sp(g, p);
                    else
        ...
        public void CalcPointAng(int index = -1)
        {
            return;
            if (index == -1)
            ...
            if (index >= Points.Count - 1)
                return;
            LPoint p0 = Points[index - 1];
            LPoint p1 = Points[index];
            LPoint p2 = Points[index + 1];
            float a1 = p0.Angle(p1);
            float a2 = p1.Angle(p2);
            float a = (a1 + a2) * .5f;
            p1.dX = -(float)Math.Cos(a);
            p1.dY = -(float)Math.Sin(a);
            p0.Spline = new QubicSpline(p0, p1);
        }
